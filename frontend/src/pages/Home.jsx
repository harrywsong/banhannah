        ...
        <div className="relative overflow-hidden">
          ...
          >
            {shuffledReviews.map((review, index) => (
              <div
                key={`home-review-${review.id}-${index}`}  // ✅ FIXED: Unique key
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex text-yellow-400">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={`home-review-${review.id}-star-${i}`}  // ✅ FIXED: Unique key
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {/* ✅ IMPROVED: Show review type with better formatting */}
                    <p className="text-sm text-gray-500">
                      {review.itemType === 'file' && '파일'}
                      {review.itemType === 'course' && '온라인 코스'}
                      {review.itemType === 'class' && '라이브 클래스'}
                      {' 리뷰'}
                    </p>
                  </div>
                  <MessageCircle className="h-5 w-5 text-gray-400" />
                </div>
                {/* ✅ ADDED: Show what item was reviewed */}
                {review.itemTitle && (
